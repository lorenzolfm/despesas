 ...
 */
export function calculateShares(monthTransactions: Transaction[]): {
    lorenzo: number;
    maria: number;
} {
    const lorenzoIncome = sumTransactions(
        filterTransactions(monthTransactions, "Lorenzo", "Income"),
    );
    const mariaIncome = sumTransactions(
        filterTransactions(monthTransactions, "Maria", "Income"),
    );
...
export function calculatePersonMonthlyTotals(
    ...
    const income = sumTransactions(
        filterTransactions(monthTransactions, owner, "Income"),
    );
    const credit = sumTransactions(
        filterTransactions(monthTransactions, owner, "Credit"),
    );
    const personal = sumTransactions(
        filterTransactions(monthTransactions, owner, "Personal"),
    );
    const settlement = sumTransactions(
        filterTransactions(monthTransactions, owner, "Settlement"),
    );

    // What this person paid for shared expenses
    const split5050Paid = sumTransactions(
        filterTransactions(monthTransactions, owner, "Split 50/50"),
    );
    const householdPaid = sumTransactions(
        filterTransactions(monthTransactions, owner, "Household"),
    );
    const paidForPartner = sumTransactions(
        filterTransactions(monthTransactions, owner, "Paid for Partner"),
    );

    // Total shared expenses (from both people)
    const totalSplit5050 = sumTransactions(
        filterTransactions(monthTransactions, undefined, "Split 50/50"),
    );
    const totalHousehold = sumTransactions(
        filterTransactions(monthTransactions, undefined, "Household"),
    );
    ...
    const partnerPaidForMe = sumTransactions(
        filterTransactions(monthTransactions, partner, "Paid for Partner"),
    );
    ...
    const realSpending =
        personal + split5050Portion + householdPortion + partnerPaidForMe;

    // Calculate category totals for this person's transactions
    const personTransactions = monthTransactions.filter(
        (tx) => tx.owner === owner,
    );
...
export function calculateCombinedMonthlyTotals(
    ...
    const lorenzo = calculatePersonMonthlyTotals(
        monthTransactions,
        "Lorenzo",
        monthKey,
    );
    const maria = calculatePersonMonthlyTotals(
        monthTransactions,
        "Maria",
        monthKey,
    );
    ...
    const totalIncome = sumTransactions(
        filterTransactions(monthTransactions, undefined, "Income"),
    );
    const totalCredit = sumTransactions(
        filterTransactions(monthTransactions, undefined, "Credit"),
    );
    const totalSplit5050 = sumTransactions(
        filterTransactions(monthTransactions, undefined, "Split 50/50"),
    );
    const totalPaidForPartner = sumTransactions(
        filterTransactions(monthTransactions, undefined, "Paid for Partner"),
    );
    const totalHousehold = sumTransactions(
        filterTransactions(monthTransactions, undefined, "Household"),
    );
    const totalPersonal = sumTransactions(
        filterTransactions(monthTransactions, undefined, "Personal"),
    );
    const totalSettlement = sumTransactions(
        filterTransactions(monthTransactions, undefined, "Settlement"),
    );

    const grandTotal =
        totalSplit5050 + totalPaidForPartner + totalHousehold + totalPersonal;
 ...
 */
export function calculateAllMonthlyTotals(
    transactions: Transaction[],
): CombinedMonthlyTotals[] {
