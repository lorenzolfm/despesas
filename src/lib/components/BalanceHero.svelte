...
<Card padding="none">
    <!-- Accent bar at top -->
    <div
        class="h-1 {balance.amount < 0.01 ? 'bg-primary' : 'bg-negative'}"
    ></div>
    ...
    <div class="p-5 sm:p-6">
        <div class="flex flex-col items-center text-center">
            {#if balance.amount < 0.01}
                <!-- Settled up state -->
                <div
                    class="w-14 h-14 rounded-full bg-primary/10 flex items-center justify-center mb-3 animate-check-scale"
                >
                    <svg
                ...
                <h2 class="text-xl font-semibold text-themed mb-0.5">
                    All settled up!
                </h2>
                <p class="text-sm text-themed-secondary">
                    You and your partner are even
                </p>
            {:else}
                <!-- Balance owed state -->
                <div class="flex items-center gap-4 mb-3">
                    ...
                    <div class="flex flex-col items-center">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            class="w-5 h-5 text-themed-tertiary"
                            viewBox="0 0 24 24"
                            fill="none"
                            stroke="currentColor"
                            stroke-width="2"
                ...
                <h2 class="text-lg font-semibold text-themed mb-1">
                    {balance.debtor} owes {balance.debtor === "Lorenzo"
                        ? "Maria"
                        : "Lorenzo"}
                </h2>
