    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-14">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-lg font-semibold text-themed leading-tight">
                        Despesas
                    </h1>
                    <p class="text-xs text-themed-tertiary">
                        ...
                        {:else}
                            <span class="inline-flex items-center gap-1.5">
                                <span
                                    class="inline-flex items-center justify-center min-w-[1.25rem] h-5 px-1.5 rounded-full bg-primary/10 text-primary text-[11px] font-semibold"
                                >
                                    {transactionCount}
                                </span>
            ...
            <div class="flex items-center gap-3">
                {#if onRefresh}
                    ...
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            class="w-4 h-4 transition-transform duration-600 {isSpinning ||
                            isLoading
                                ? 'animate-spin'
                                : ''}"
                            viewBox="0 0 24 24"
                ...
                {#if session?.user}
                    <div class="hidden sm:flex items-center gap-2">
                        {#if session.user.image}
                            ...
                            />
                        {/if}
                        <span class="text-xs text-themed-secondary font-medium"
